...
class AccessPermissionDialogUtil : PermissionDialogUtil {
     ...
     * 1
     */
    override fun initAccessPermissionDialog(
        context: Context,
        jumpCallback: () -> Unit,
        cancelCallback: () -> Unit
    ): AccessPermissionDialogUtil {
        dialog = Dialog(context, R.style.AccessPermissionDialog)
        view = LayoutInflater.from(context).inflate(R.layout.dialog_access_permission, null)
